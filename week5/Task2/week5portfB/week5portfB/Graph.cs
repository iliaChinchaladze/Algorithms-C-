    ...
    class Graph<T> where T : IComparable
    {
...

        {
            ...
            while (toVisit.Count != 0)
            {
                ...
                {
                    if (toVisit.Contains(id) == false && visited.Contains(id) == false)
                    { toVisit.Push(id); }

                }
                
                // Get ID of current node from toVisit (use “pop”)
                // Store ID of the current node in “visited” 

                // Get adjacency list of the current node (method from GraphNode)
                // Add to toVisit (use “push”) each node n in the adjacency list of the current node 
